...
MINUTE=$(echo "${GITHUB_REPOSITORY}" | md5sum | tr -d -c 0-9 | xargs -I{} echo {}123 | cut -c 1-8 | xargs -I{} bash -c 'echo $(({} % 60))')
# shellcheck disable=SC2016
HOUR=$(echo "${GITHUB_REPOSITORY}" | md5sum | tr -d -c 0-9 | xargs -I{} echo {}123 | cut -c 1-8 | xargs -I{} bash -c 'echo $(({} % 24))')
# shellcheck disable=SC2016
DATE=$(echo "${GITHUB_REPOSITORY}" | md5sum | tr -d -c 0-9 | xargs -I{} echo {}123 | cut -c 1-8 | xargs -I{} bash -c 'echo $(({} % 28 + 1))')
